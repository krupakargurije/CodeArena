            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-[#12121a]/80 backdrop-blur-xl rounded-xl border border-white/5 p-6">
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h2 className="text-white font-semibold text-lg">Problems (preview)</h2>
                                    <p className="text-dark-text-tertiary text-xs">Quick filter. Full list on the Problems page.</p>
                                </div>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="pl-9 pr-4 py-2 bg-[#0a0a0f] border border-white/10 rounded-lg text-sm text-white placeholder-dark-text-tertiary focus:outline-none focus:ring-1 focus:ring-brand-blue/50 w-56"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                {problems.map((problem) => (
                                    <div
                                        key={problem.id}
                                        className="flex items-center justify-between p-4 rounded-xl bg-[#0a0a0f]/50 border border-white/5 hover:border-white/10 transition-all group"
                                    >
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-1.5">
                                                <span className="text-dark-text-tertiary text-xs font-mono">CA {problem.id}</span>
                                                <span className={`px-2 py-0.5 rounded border text-xs font-medium ${getDifficultyStyle(problem.difficulty)}`}>
                                                    {problem.difficulty}
                                                </span>
                                            </div>
                                            <h3 className="text-white font-medium text-sm mb-2 group-hover:text-brand-blue transition-colors">
                                                {problem.title}
                                            </h3>
                                            <div className="flex gap-2">
                                                {problem.tags.map((tag, idx) => (
                                                    <span key={idx} className="px-2 py-0.5 rounded bg-white/5 text-dark-text-tertiary text-xs">
                                                        {tag}
                                                    </span>
                                                ))}
                                            </div>
                                        ...
                                        <div className="flex items-center gap-6">
                                            <div className="text-right">
                                                <div className="text-dark-text-secondary text-sm">{problem.acceptance} acceptance</div>
                                                <div className="text-dark-text-tertiary text-xs">{problem.points} pts</div>
                                            </div>
                                            <Link
                                                to={`/problems/${problem.id}`}
                                                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-white/5 text-white text-sm font-medium hover:bg-white/10 transition-colors"
                                            >
                                                Open
                            ...
                            <div className="mt-6 text-center">
                                <Link to="/problems" className="text-dark-text-secondary hover:text-white text-sm transition-colors">
                                    View all problems
                                </Link>
                            </div>
                        </div>
                    ...
                    <div className="space-y-6">
                        {/* Contests */}
                        <div className="bg-[#12121a]/80 backdrop-blur-xl rounded-xl border border-white/5 p-5">
                            <div className="mb-4">
                                <h2 className="text-white font-semibold">Contests</h2>
                                <p className="text-dark-text-tertiary text-xs">Upcoming events and quick joins.</p>
                            </div>

                            <div className="space-y-3">
                                {contests.map((contest) => (
                                    <div key={contest.id} className="p-4 rounded-xl bg-[#0a0a0f]/50 border border-white/5">
                                        <div className="flex items-start justify-between mb-2">
                                            <div>
                                                <h3 className="text-white font-medium text-sm">{contest.name}</h3>
                                                <p className="text-dark-text-tertiary text-xs">
                                                    Starts {contest.startsIn} · {contest.duration}
                                                </p>
                                            </div>
                                            <span className="text-dark-text-tertiary text-xs font-mono">{contest.code}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            ...
                                            <button className="px-3 py-1 rounded-lg bg-white/5 text-white text-xs font-medium hover:bg-white/10 transition-colors">
                                                Join
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className="bg-[#12121a]/80 backdrop-blur-xl rounded-xl border border-white/5 p-5">
                            <div className="mb-4">
                                ...
                                <p className="text-dark-text-tertiary text-xs">Top performers this season.</p>
                            </div>

                            {loading ? (
                                <div className="text-center py-8 text-dark-text-tertiary">Loading...</div>
                            ) : (
                                <div className="space-y-2">
                                    {leaderboard.map((user, index) => (
                                        <div key={user.id || index} className="flex items-center gap-3 p-3 rounded-xl bg-[#0a0a0f]/50 border border-white/5">
                                            {/* Rank Badge */}
